...
Instance.prototype.registerHelpers = function (directory, done) {
  var handlebars = this.handlebars;

  var register = function(filepath, done) {
    var isValidHelper = /\.(js)$/.test(filepath);
...
Instance.prototype.registerPartials = function (directory, done) {
  ...
  var register = function(filepath, done) {
    var isValidTemplate = /\.(template)$/.test(filepath);
    ...
    fs.readFile(filepath, 'utf8', function(err, data) {
      if (!err) {
        var ext = path.extname(filepath);
        var templateName = path.basename(filepath)
